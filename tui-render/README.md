...
- **Performance bounds** — each streaming Markdown block advances an append-only complete-line collector, retains stable parsed blocks and physical-row references, and reparses only the ambiguous suffix; reference definitions, visualization directives, render-scope changes, and unprovable locality take an explicit safe full-recompute path. A fence-aware table scanner holds back only the active table from its header, while adaptive layout preserves compact values, shrinks token-heavy and narrative columns, and selects key/value records when required. `TranscriptRenderStore`, projector caches, and syntax-token caches are bounded and reconstructable from canonical source. A shared frame arbiter gives latest-target scrolling priority over a bounded smooth/catch-up presentation queue. `VisibleFrameSnapshot` suppresses unchanged transcript writes and compares stale cleanup by terminal coordinate, so it clears shortened or vacated ranges without erasing a different row that moved into the same position. `pnpm run test:tui:perf` owns the strict real-PTY long-Markdown, growing-table, 10,000-row scrolling, resize, and slow-sink targets.
