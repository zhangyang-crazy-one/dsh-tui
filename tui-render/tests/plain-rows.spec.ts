...
it('applies toolBg background and backgroundColumns to materialized lines', () => {
  const cache = new PlainTextRowCache({ maxRows: 100, maxBytes: 4096 })
  const rows = cache.rows('thought', '思考过程', 40, 80)
  expect(rows.length).toBe(1)
  expect(rows.at(0)?.background).toBe('toolBg')
  expect(rows.at(0)?.backgroundColumns).toBe(80)
  expect(rows.at(0)?.spans[0]?.token).toBe('accentText')
  expect(rows.at(0)?.spans[1]?.token).toBe('fgDim')
})
