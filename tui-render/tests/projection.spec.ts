...
describe('createProjector', () => {
  ...
  it('anchors an away-from-bottom viewport while tracking unseen rows', () => {
    const projector = createProjector()
    for (let seq = 1; seq <= 100; seq += 1) {
      projector.push(event(seq, 'user/message', {
        content: [{ type: 'text', text: `row-${seq}` }],
        source: { kind: 'user' },
      }))
    }
    projector.setScroll(1_000)
    const before = projector.snapshot()
    expect(before.scrollOffset).toBe(40)
    expect(before.follow).toBe(false)
    const beforeVisible = before.history.slice(
      before.history.length - 60 - before.scrollOffset,
      before.history.length - before.scrollOffset,
    )

    projector.push(event(101, 'user/message', {
      content: [{ type: 'text', text: 'row-101' }],
      source: { kind: 'user' },
    }))
    const anchored = projector.snapshot()
    const anchoredVisible = anchored.history.slice(
      anchored.history.length - 60 - anchored.scrollOffset,
      anchored.history.length - anchored.scrollOffset,
    )
    expect(anchored.scrollOffset).toBe(41)
    expect(anchored.unseenCount).toBe(1)
    expect(anchoredVisible[0]?.id).toBe(beforeVisible[0]?.id)
    expect(anchoredVisible.at(-1)?.id).toBe(beforeVisible.at(-1)?.id)

    projector.setScroll(-100)
    expect(projector.snapshot()).toMatchObject({
      scrollOffset: 0,
      follow: false,
      unseenCount: 1,
    })
    projector.followLatest()
    expect(projector.snapshot()).toMatchObject({
      scrollOffset: 0,
      follow: true,
      unseenCount: 0,
  ...
  it('keeps explicit detach state at the live edge until a latest action reattaches', () => {
    const projector = createProjector()
    projector.setScroll(1)
    expect(projector.snapshot()).toMatchObject({ follow: false, scrollOffset: 0 })

    projector.push(event(1, 'user/message', {
      content: [{ type: 'text', text: 'detached append' }],
      source: { kind: 'user' },
    }))
    expect(projector.snapshot()).toMatchObject({
      follow: false,
  ...
  it('jumps to the oldest window without losing explicit detach state', () => {
    const projector = createProjector()
    for (let seq = 1; seq <= 100; seq += 1) {
      projector.push(event(seq, 'user/message', {
        content: [{ type: 'text', text: `row-${String(seq)}` }],
        source: { kind: 'user' },
      }))
    }
    projector.scrollToOldest()
    expect(projector.snapshot()).toMatchObject({
      follow: false,
      scrollOffset: 40,
    })
